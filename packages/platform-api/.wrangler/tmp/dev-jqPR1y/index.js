...
// .wrangler/tmp/bundle-MvMSOt/middleware-loader.entry.ts
init_modules_watch_stub();
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_process();
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_console();
init_performance2();

// .wrangler/tmp/bundle-MvMSOt/middleware-insertion-facade.js
init_modules_watch_stub();
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_process();
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_console();
init_performance2();
...
var TOOLS = [
  {
    ...
    function: {
      name: "get_stats",
      description: "Get aggregated event statistics (clicks, postbacks, impressions) for the last N hours. Returns hourly counts and summary totals by event_type.",
      parameters: {
        type: "object",
        properties: {
          hours: {
  ...
  },
  {
    ...
    function: {
      name: "list_events",
      description: "List recent tracking events. Returns events with event_type, timestamp, IP, user_agent, and custom params.",
      parameters: {
        type: "object",
        properties: {
          limit: {
          ...
          tu_id: { type: "string", description: "Filter by tracking URL ID" },
          param_key: {
            type: "string",
            description: "Filter by custom param key (e.g. 'sub1', 'geo')"
          },
          param_value: {
            type: "string",
    ...
    function: {
      ...
      parameters: {
        ...
        properties: {
          group_by: {
            type: "string",
            description: "The param key to group by (e.g. 'sub1', 'geo', 'campaign_id')"
          },
          hours: {
            type: "number",
    ...
    function: {
      ...
      parameters: {
        ...
        properties: {
          ...
          tu_id: { type: "string", description: "Filter by tracking URL ID" },
          param_key: { type: "string", description: "Additional param filter" },
          param_value: { type: "string", description: "Param filter value" }
        }
      }
    }
  },
  {
    ...
    function: {
      ...
      parameters: {
        type: "object",
        properties: {
          limit: { type: "number", description: "Max URLs (default: 50)" }
        }
      }
    }
  },
  {
    ...
    function: {
      name: "list_tenants",
      description: "List all tenants on the platform with id, name, plan, and key_prefix.",
  ...
  },
  {
    ...
    function: {
      name: "list_webhooks",
      description: "List registered webhooks with URL, event_types, active status.",
      parameters: { type: "object", properties: {} }
    }
  },
  {
    ...
    function: {
      name: "health_check",
      description: "Check if the Platform API is healthy.",
      parameters: { type: "object", properties: {} }
    }
  }
];
function buildToolPath(toolName, args) {
  const params = new URLSearchParams();
  switch (toolName) {
    case "get_stats":
      if (args.hours) params.set("hours", String(args.hours));
      if (args.event_type) params.set("event_type", String(args.event_type));
      if (args.tu_id) params.set("tu_id", String(args.tu_id));
      return `/api/events/stats?${params.toString()}`;
    case "list_events":
      if (args.limit) params.set("limit", String(args.limit));
      if (args.event_type) params.set("event_type", String(args.event_type));
      if (args.tu_id) params.set("tu_id", String(args.tu_id));
      if (args.param_key) params.set("param_key", String(args.param_key));
      if (args.param_value) params.set("param_value", String(args.param_value));
      return `/api/events?${params.toString()}`;
    case "get_breakdown":
      params.set("group_by", `param:${args.group_by}`);
      if (args.hours) params.set("hours", String(args.hours));
      if (args.event_type) params.set("event_type", String(args.event_type));
      if (args.tu_id) params.set("tu_id", String(args.tu_id));
      return `/api/events/stats?${params.toString()}`;
    case "match_events":
      if (args.trigger) params.set("trigger", String(args.trigger));
      if (args.result) params.set("result", String(args.result));
      if (args.on) params.set("on", String(args.on));
      if (args.hours) params.set("hours", String(args.hours));
      if (args.limit) params.set("limit", String(args.limit));
      if (args.tu_id) params.set("tu_id", String(args.tu_id));
      if (args.param_key) params.set("param_key", String(args.param_key));
      if (args.param_value) params.set("param_value", String(args.param_value));
      return `/api/events/match?${params.toString()}`;
    case "list_tracking_urls":
      if (args.limit) params.set("limit", String(args.limit));
      return `/api/tracking-urls?${params.toString()}`;
    case "list_tenants":
      return "/api/tenants";
    case "list_webhooks":
      return "/api/webhooks";
...
__name(buildToolPath, "buildToolPath");
var _app = null;
...
async function executeTool(toolName, args, authHeader, env2) {
  const path = buildToolPath(toolName, args);
  if (!path) return JSON.stringify({ error: `Unknown tool: ${toolName}` });
...
var SYSTEM_PROMPT = `You are an analytics assistant for a tracking platform. You help users understand their click, postback, and impression data using the available tools.
