...
// .wrangler/tmp/bundle-L0T4CD/middleware-loader.entry.ts
init_modules_watch_stub();
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_process();
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_console();
init_performance2();

// .wrangler/tmp/bundle-L0T4CD/middleware-insertion-facade.js
init_modules_watch_stub();
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_process();
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_console();
init_performance2();
...
events.post("/query", async (c) => {
  ...
  const fullSql = hasLimit ? `WITH scoped AS (SELECT * FROM events WHERE ${scopeWhere}) ${userSql}` : `WITH scoped AS (SELECT * FROM events WHERE ${scopeWhere}) ${userSql} LIMIT ${limit}`;
  let rw = null;
...
// src/routes/ingest-tokens.ts
init_modules_watch_stub();
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_process();
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_console();
init_performance2();
var ingestTokens = new Hono2();
function generateIngestToken(keyPrefix) {
  return `pt_${keyPrefix}_${randomAlphanumeric(32)}`;
}
__name(generateIngestToken, "generateIngestToken");
ingestTokens.post("/", async (c) => {
  ...
  const keyPrefix = tenant.rows[0].key_prefix;
  const id = generateId();
  const plainToken = generateIngestToken(keyPrefix);
  const tokenHash = await sha256(plainToken);
  const name = body.name || "Default";
  const pluginId = body.plugin_id || null;
  let expiresAt = null;
  if (body.expires_in_days) {
    expiresAt = Math.floor(Date.now() / 1e3) + body.expires_in_days * 86400;
  }
  await db.execute({
...
var ingest_tokens_default = ingestTokens;
...
var similar_events = {
  name: "similar_events",
  description: "Find events similar to a given event or text query using vector similarity search (LanceDB). Useful for anomaly detection, fraud pattern discovery, and 'more like this' features. NOTE: This endpoint is currently stubbed \u2014 LanceDB integration is pending.",
  parameters: {
    event_id: {
      type: "string",
...
var ai_chat = {
  name: "ai_chat",
  description: "Multi-turn AI conversation about your data. Send a conversation history and the AI will generate and execute queries as needed, returning both a natural language response and query results. Supports follow-up questions and context from previous messages.",
  parameters: {
    messages: {
      type: "array",
      description: "Conversation messages array. Each message has 'role' ('user' or 'assistant') and 'content' (text).",
      items: { type: "string" },
      required: true
    },
    key_prefix: {
...
app.route("/api/ingest-tokens", ingest_tokens_default);
