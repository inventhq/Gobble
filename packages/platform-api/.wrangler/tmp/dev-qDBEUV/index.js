...
// .wrangler/tmp/bundle-cij1j7/middleware-loader.entry.ts
init_modules_watch_stub();
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_process();
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_console();
init_performance2();

// .wrangler/tmp/bundle-cij1j7/middleware-insertion-facade.js
init_modules_watch_stub();
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_process();
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_console();
init_performance2();
...
events.get("/history", async (c) => {
  ...
  const tenantFilter = keyPrefix || "_all";
...
events.get("/stats/merged", async (c) => {
  ...
  if (dateFrom && dateTo) {
    ...
    coldFrom = epochToDate(coldCutoffSecs);
    coldTo = epochToDate(hotBoundarySecs);
  ...
  let coldResult = null;
  if (polarsUrl && coldFrom && coldTo) {
    const tenantFilter = keyPrefix || "_all";
    coldResult = await queryPolars(polarsUrl, {
      tenant_id: tenantFilter,
      tu_id: tuId || void 0,
  ...
  if (coldResult?.rows) {
    for (const row of coldResult.rows) {
      const key = row.event_type;
  ...
  return c.json({
    ...
    hours,
    hot_hours: hotHours,
    cold_range: coldFrom && coldTo ? { from: coldFrom, to: coldTo } : null,
    sources: {
      hot: !!hotResult,
      cold: !!coldResult,
      hot_partitions: hotResult ? "risingwave" : null,
      cold_partitions: coldResult?.partitions_scanned || 0,
      cold_query_ms: coldResult?.query_ms || 0
    },
    server_time: Date.now()
  });
});
